...
\begin{description}[style=nextline]
    \item[Decision variable $\vec{x} \in \RR^n$] The quantities we choose. We stack all decision variables into a single vector. For example, if we choose amounts $x_1, x_2, x_3$, then $\vec{x} = (x_1, x_2, x_3)^\top \in \RR^3$.
    \item[Objective function $f_0: \RR^n \to \RR$] The quantity we want to minimize. Common objectives include cost, error, distance, or negative profit.
    \item[Inequality constraints $f_i(\vec{x}) \le 0$] Restrictions that must hold with ``$\le$'' on the right. The standard form requires the right-hand side to be zero.
    \item[Equality constraints $h_j(\vec{x}) = 0$] Restrictions that must hold with exact equality.
    \item[Feasible set $\Omega$] All points satisfying every constraint. If $\Omega = \emptyset$, the problem is \textbf{infeasible}.
    \item[Solution $\vec{x}^\star$] A feasible point achieving the smallest objective value.
\end{description}
...
\begin{claim}
The projection $\vec{z}$ minimizes $\|\vec{y} - \vec{u}\|_2$ over all $\vec{u} \in \Range(A)$.
\end{claim}

\begin{proof}
Let $\vec{u} \in \Range(A)$ be any point in the column space. We want to show $\|\vec{y} - \vec{u}\|_2 \ge \|\vec{e}\|_2$.
...
with equality if and only if $\vec{u} = \vec{z}$.
\end{proof}
